...
public class LoginStepDefinitions extends Utils {
	...
	public void user_calls_api_with_http_method(String resource, String httpMethod) {
		...
		if (httpMethod.equalsIgnoreCase("POST")) {
			TestContext.response = TestContext.request.when().log().ifValidationFails()
					.post(apiResources.getAPIResource());
	...
	public void api_success_with_status_code(Integer expectedStatusCode) {
		...
		if (res.contains("orders")) {
			...
			CreateOrderResponse createOrderResponse = TestContext.response.as(CreateOrderResponse.class);
			TestContext.orders = createOrderResponse.getOrders().get(0);
		}
